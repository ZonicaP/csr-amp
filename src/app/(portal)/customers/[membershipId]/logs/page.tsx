...
export default async function CustomerLogsPage({ params }: { params: Promise<{ membershipId: string }> }) {
  ...
  return (
    <Stack spacing={1}>
      {calls.length > 0 ? (
        <Stack spacing={1}>
          <Typography sx={{ color: "#717680", fontSize: 14, fontWeight: 600 }}>Calls</Typography>
          {calls.map((call) => (
            <Paper key={call.reference} elevation={0} sx={{ px: 2, py: 1.25, border: "1px solid #E5E7EB", borderRadius: 3 }}>
              <Stack direction="row" sx={{ justifyContent: "space-between", gap: 1, alignItems: "baseline" }}>
                ...
                <Typography sx={{ color: "#717680", fontSize: 13 }}>{date.format(call.startedAt)}</Typography>
              </Stack>
              <Typography sx={{ color: "#717680", fontSize: 14 }}>
                {call.csr.displayName} · {call.status === "CALLBACK" ? "Callback" : call.status === "OPEN" ? "Open" : "Closed"}
              </Typography>
            </Paper>
          ))}
        </Stack>
      ) : null}
      {customer.events.length === 0 && calls.length === 0 ? <Typography>No events on this account.</Typography> : null}
