...
- Customer email goes to the member. Seeded `@example.com` addresses cannot be delivered, so those messages go to the signed-in CSR and the footnote says so. The payment link, the offer, the plate request, the refund notice, and the cancellation confirmation all follow that rule.
